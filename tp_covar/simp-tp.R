...
sig.index
for (r in 1:Rep){
  if (r%%100==0) cat("the loop is", r ,"\n")
  ##generate data
  SNP =gen.MultiSNP(MAF.vec = MAF.vec,sig.mat = sig.mat,n.case=n.case,
                    beta0 = beta0,n.control = n.control,N=N,bet = bet,gamma = gamma,
                    gen.model=gen.model)
  data = SNP$df
  y=data$y
  G = as.matrix(data[,2:(m+1)])
  Z = as.matrix(data$z)
  result[,r]=pvalue_tp(y,G,Z,B=500)
  sim_data[[r]] = data
}
proc.time()-ptm
...
save(sim_data,file = paste0("sim_data/m",m,"_gen",nlociR,"R",nlociD,"D",nlociA,"A","_sm",sig.index,"_beta",max(bet),".Rdata"))
