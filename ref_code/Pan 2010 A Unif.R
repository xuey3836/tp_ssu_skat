...
### g-inv for a possibly singular cov matrices: 
###  a lower-bound of the eigen-values of CovS is set at 1e-8 s.t. its inverse is well defined.   1/11/09
...
SumSqUs<-function(Y, X, B=1000){
  ...
  ##distr of Tg1 is sum of cr Chisq_1:
  # 1/diag(CovS): can be infinite or missing!
  #cr<-eigen(CovS %*% diag(1/diag(CovS)), only.values=TRUE)$values
  diagCovS<-diag(CovS)
  diagCovSinv<-ifelse(abs(diagCovS) >1e-8, 1/diagCovS, 1e-8)
  cr<-eigen(CovS %*% diag(diagCovSinv), only.values=TRUE)$values
  ##approximate the distri by alpha Chisq_d + beta:
